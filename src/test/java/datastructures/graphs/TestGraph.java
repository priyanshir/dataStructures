...
public class TestGraph extends BaseTest {
    ...
    void testFindMst2() {
        ...
        IList<SimpleEdge<String>> edges = new DoubleLinkedList<>();
        edges.add(edge("a", "b", 2)); //0 -
        edges.add(edge("a", "c", 5)); //1 -
        edges.add(edge("a", "d", 1)); //2 -
        edges.add(edge("a", "f", 9)); //3
        edges.add(edge("b", "c", 8)); //4
        edges.add(edge("b", "d", 4)); //5
        edges.add(edge("b", "e", 3)); //6 -
        edges.add(edge("d", "f", 6)); //7 -
        edges.add(edge("e", "f", 7)); //8

        Graph<String, Integer> graph = buildGraphWithData(vertices, edges);

        ISet<Integer> mst = Edge.edgesToSetOfData(graph.findMinimumSpanningTree());
        assertThat(mst, is(setContaining(0, 1, 2, 6, 7)));
    }
    ...
    @Test
    void testBasicGraphState() { //tested internal state using debugger (find out how to declare internal fields)
        Graph<String, Void> graph = new Graph<>();
        graph.addVertex("a");
        graph.addVertex("b");
        graph.addVertex("c");
        graph.addEdge("a", "a", 3.0, null);
        graph.addEdge("a", "c", 2.0, null);
    }
    @Test
    void testAllExceptions() {
        ...
        assertThrows(IllegalArgumentException.class, () -> graph.addEdge("d", "b", 3.0));
        assertThrows(IllegalArgumentException.class, () -> graph.addEdge("a", "f", 1.0));
        assertThrows(IllegalArgumentException.class, () -> graph.findShortestPathBetween("a", null));
        assertThrows(IllegalArgumentException.class, () -> graph.findShortestPathBetween(null, "a"));
        assertThrows(IllegalArgumentException.class, () -> graph.findShortestPathBetween("e", "a"));
        assertThrows(IllegalArgumentException.class, () -> graph.findShortestPathBetween("f", "a'"));
    }
}
